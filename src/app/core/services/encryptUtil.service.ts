...
export class EncryptUtilService {
    ...
    AesEncrypt(text: string, key: string, iv: string) {
        ...
        const ciphertext = CryptoJS.AES.encrypt(text, key2, { iv: iv2, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 });
    ...
    AesDecrypt(text: string, key: string, iv: string) {
        let key2 = CryptoJS.enc.Utf8.parse(key);
        let iv2 = CryptoJS.enc.Utf8.parse(iv);
        const bytes = CryptoJS.AES.decrypt(text.toString(), key2, { iv: iv2, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 });
