...
export class DataSearchbarComponent implements OnInit {
    ...
    selectList: Array<{ label: string, values: Array<{ property: string, value: string }> }> = new Array();
    ...
    ngOnInit() {
        ...
        if (this.searchSets && this.searchSets.length > 0) {
            this.inputTypeList = this.searchSets.map(s => {
                ...
                return Object.assign({ label: mapColumn ? mapColumn.value : s.property, apiProperty: s.apiProperty }, s.InputOpts);
            });
            this.validateForm = this.fb.group(myForm);
            this.optionList = this.inputTypeList.filter((value) => (value.type === 'text') || (value.type === 'textarea') || (value.type === 'number'));
    ...
    submitForm(mode: string) {
        ...
        this.dataDriveService.searchData(this.dataDrive, send).subscribe((c: any[]) => {
