...
export class MyFlexPipe implements PipeTransform {
  ...
  transform(value: any, args?: { name: string, params: any[] }): any {
    if (args && this[args.name]) {
      ...
      return res instanceof Observable || res instanceof Promise ? res : Observable.of(res);
    } else {
    ...
    if (api) {
      ...
      if (cache) {
        ...
        if (!this.lazyLoadRequest[api]) {
          this.lazyLoadRequest[api] = this.dataDriveService.lazyLoad(api).map((r: any[]) => {
            if (isArray(r)) {
          ...
          }).catch((err => Observable.of(target))).toPromise();
        }
  ...
  empno(target: string, format?: string) {
    ...
    if (cache && cache.data && cache.data[target]) {
      ...
      if (!this.colleagueRequest[target]) {
        this.colleagueRequest[target] = this.appService.getColleague(target).map((data: any) => {
          if (data.length === 1) {
            const val = data[0];
