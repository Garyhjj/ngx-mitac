...
export class ITServerRateComponent implements OnInit {
  ...
  constructor(private reservationITService: ReservationITService) { }

  ngOnInit() {
  }

  getDataDrive(d: DataDrive) {
    d.beforeInitTableData((ls) => ls.sort((a, b) => {
  ...
  async toRateDetail(app: any) {
    ...
    this.yinxiangList = [];
    this.commentList = [];
    Observable.forkJoin(this.reservationITService.getPersonImpression(empno, 6),
