...
export class ITServerBoardComponent implements OnInit {
  ...
  async getDataDrive(d: DataDrive) {
    ...
    d.beforeInitTableData(ds => ds.filter(l => {
      if (l.DEPT_ID !== DEPT_ID) {
        ...
        const date = moment(l.SERVICE_DATE).format('YYYY-MM-DD') + ' ' + l.END_TIME;
    ...
    }).sort((a, b) => sortUtils.byDate(moment(a.SERVICE_DATE).format('YYYY-MM-DD') + ' ' + a.START_TIME, moment(b.SERVICE_DATE).format('YYYY-MM-DD') + ' ' + b.END_TIME)));
    d.changeSearchWay(() => this.reservationITService.getUndoneReservationList());
    d.afterDataInit((allData) => this.util.playAudio(`有${allData.length}条超期或快超期的服务`, { single: true }));
